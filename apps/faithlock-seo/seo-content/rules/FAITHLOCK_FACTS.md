...
| Prix / abonnement (RevenueCat) | ◐ PARTIEL — à confirmer par le fondateur | prix à reprendre tels que déjà publiés sur le site (voir section claims ci-dessous) uniquement, ne pas en inventer de nouveaux | `lib/services/subscription/revenuecat_service.dart` (intégration RevenueCat réelle : achats, restauration, offres promo, essai gratuit lié à `introductoryPrice` du store — prix non codés en dur, viennent du store), `lib/features/paywall/controllers/paywall_controller.dart` |
